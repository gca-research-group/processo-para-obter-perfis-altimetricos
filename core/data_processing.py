...
# 3 ===== FUNÇÕES PARA CÁLCULO DE DISTÂNCIA =====
...
# 5 ===== FUNÇÕES PARA CÁLCULO DE MÉTRICAS =====
...
def calcular_metricas_suavizacao(
    altitudes_originais, altitudes_suavizadas
) -> Dict[str, float]:
    """Calcula métricas de qualidade da suavização"""
    return {
        "RMSE": np.sqrt(
            mean_squared_error(altitudes_originais, altitudes_suavizadas)
        ),  # Raiz do erro quadrático médio
        "MAE": mean_absolute_error(
            altitudes_originais, altitudes_suavizadas
        ),  # Erro absoluto médio
        "MaxError": np.max(
            np.abs(altitudes_originais - altitudes_suavizadas)
        ),  # Máximo erro absoluto
        "StdDev_Original": np.std(altitudes_originais),  # Desvio padrão original
        "StdDev_Suavizado": np.std(altitudes_suavizadas),  # Desvio padrão suavizado
        "Reducao_Ruido": 1
        - (
            np.std(altitudes_suavizadas) / np.std(altitudes_originais)
        ),  # Redução de ruído
    }
...
def analisar_inclinacoes(distancias, altitudes) -> Dict[str, float]:
    ...
    diff_alt = np.diff(altitudes)  # Diferença de altitudes
    diff_dist = np.diff(distancias)  # Diferença de distâncias
    inclinacoes = np.degrees(np.arctan(diff_alt / diff_dist))  # Converte para graus

    return {
        "Media_Inclinacao": np.mean(np.abs(inclinacoes)),  # Média absoluta
        "Max_Inclinacao": np.max(np.abs(inclinacoes)),  # Máxima absoluta
        "Min_Inclinacao": np.min(np.abs(inclinacoes)),  # Mínima absoluta
        "Std_Inclinacao": np.std(inclinacoes),  # Desvio padrão
    }
...
def gerar_metricas_finais(
    ...
    """Gera DataFrame com métricas para dados calculados"""
    metricas = []

    # Métricas para dados calculados
    metricas.append(
        {
            "Tipo": "Calculado",
            "Comparacao": "Suavizado vs Original",
            **calcular_metricas_suavizacao(
                df_calculado["Altitude"],
                df_calculado["Altitude_Suavizada"],
            ),
            **analisar_inclinacoes(
                df_calculado["Distancia_Acumulada"],
                df_calculado["Altitude_Suavizada"],
            ),
        }
    )
